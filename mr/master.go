...
func (m *Master) CompleteReduceTask(args *CompleteReduceArgs, reply *Reply) error {
	m.mu.Lock()
	// fmt.Printf("Task %d complete\n", args.Id)
	task := m.reduce_status[args.Id]
	task.status = "completed"
	m.reduce_status[args.Id] = task
	m.mu.Unlock()
	m.UpdateReduceStatus()
	return nil
}
...
func MakeMaster(files []string, nReduce int) *Master {
	...
	for n, f := range files {
		...
		//m.mapch <- map_task
		m.map_status[map_task.id] = map_task
	}
	for i := 0; i < nReduce; i++ {
